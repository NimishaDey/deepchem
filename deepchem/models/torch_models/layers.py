...
class WeaveLayer(nn.Module):
  ...
  def forward(self, inputs):
    ...
    AA = torch.matmul(
        torch.from_numpy(atom_features).type(self.W_AA.dtype),
        self.W_AA) + self.b_AA
    if self.batch_normalize:
      AA = self.AA_bn(AA)
    AA = activation(AA)
    PA = torch.matmul(
        torch.from_numpy(pair_features).type(self.W_PA.dtype),
        self.W_PA) + self.b_PA
    if self.batch_normalize:
      PA = self.PA_bn(PA)
    PA = activation(PA)
    ...
    if self.update_pair:
      ...
      AP_ji = torch.matmul(
          torch.reshape(
              torch.gather(atom_features, torch.reverse(atom_to_pair, [1])),
      ...
      if self.batch_normalize:
        PP = self.PP_bn(PP)
      PP = activation(PP)
      P = torch.matmul(torch.concat([AP_ij + AP_ji, PP], 1),
                       self.W_P) + self.b_P
      if self.batch_normalize:
        P = self.P_bn(P)
      P = activation(P)
